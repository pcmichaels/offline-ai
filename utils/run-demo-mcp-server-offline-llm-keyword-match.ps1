#!/usr/bin/env pwsh
# run-demo-mcp-server-offline-llm-keyword-match.ps1 - MCP Server (keyword match) demo
# Idempotent: safe to re-run. Restores caller's working directory on exit.
#
# Usage:
#   .\run-demo-mcp-server-offline-llm-keyword-match.ps1              # MCP server only (Cursor/VS Code)
#   .\run-demo-mcp-server-offline-llm-keyword-match.ps1 --standalone # Full demo: StandaloneHost + MCP server

$ErrorActionPreference = "Stop"

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Split-Path -Parent $scriptDir
$projectRoot = Join-Path (Join-Path $repoRoot "src") "mcp-server-offline-llm-keyword-match"
...
if ($standalone) {
    ...
    Write-Host "Running MCP + LM Studio demo (keyword match)..." -ForegroundColor Cyan
    ...
    Write-Host "Running MCP Server Offline LLM (keyword)..." -ForegroundColor Cyan
