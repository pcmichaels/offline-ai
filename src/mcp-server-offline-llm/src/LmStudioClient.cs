...
public sealed class LmStudioClient : IDisposable
{
    ...
    /// <summary>Chat for intent classification. Uses low temperature for deterministic JSON output.</summary>
    public async Task<string> ChatForClassificationAsync(IReadOnlyList<object> messages, CancellationToken ct = default)
    {
        var model = await ResolveModelAsync(ct);
        var request = new { model, messages, temperature = 0.1, max_tokens = 256 };

        var requestJson = JsonSerializer.Serialize(request, new JsonSerializerOptions { WriteIndented = false });
        _logger?.LogRequest("POST", "/v1/chat/completions (classification)", requestJson);
