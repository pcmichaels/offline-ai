...
public static class FileTools
{
    ...
    public static string CreateFile(
        ...
        string content)
    {
        logger.LogToolCall("CreateFile", $"[INVOKED] path={path} contentLen={content.Length}", "(executing...)");
    ...
    public static string AppendToFile(
        ...
        string content)
    {
        logger.LogToolCall("AppendToFile", $"[INVOKED] path={path} contentLen={content.Length}", "(executing...)");
        var resolvedPath = Path.GetFullPath(path);
        var directory = Path.GetDirectoryName(resolvedPath);
        if (!string.IsNullOrEmpty(directory))
            Directory.CreateDirectory(directory);
